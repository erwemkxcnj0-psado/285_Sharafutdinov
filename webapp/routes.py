...
from .services import load_rows_from_file, detect_import_type
...
def create_request():
    ...
    if request.method == "POST":
        ...
        status = request.form.get("status", "Новая").strip()
...
def edit_request(request_id):
    ...
    if request.method == "POST":
        ...
        item.status = request.form.get("status", "Новая").strip()
...
def statistics():
    ...
    new_requests = Request.query.filter_by(status="Новая").count()
    in_progress_requests = Request.query.filter_by(status="В работе").count()
    done_requests = Request.query.filter_by(status="Завершена").count()
    ...
    completed_items = Request.query.filter_by(status="Завершена").all()
        ...
        try:
            ...
            if import_type == "users":
                ...
                for row in rows:
                    ...
                    client_name=client_name,
                    client_phone=client_phone,
                    status=str(row.get("requestStatus", "Новая")).strip(),
                    planned_end_date=planned_end_date,
                    specialist_id=specialist_id
                    )
